...
function installProjectHarnessScript(projectRoot) {
  const source = path.join(ROOT, "skills", "feature-development-harness", "scripts", "harness_session.py");
  const target = path.join(projectRoot, ".harness", "scripts", "harness_session.py");
  if (!exists(source)) throw new Error(`Missing bundled harness script: ${source}`);
  const existed = exists(target);
  mkdirp(path.dirname(target));
  fs.copyFileSync(source, target);
  fs.chmodSync(target, 0o755);
  return `${existed ? "updated" : "created"} ${target}`;
}

function projectInstructionBlock({ workflow, architectureSkill }) {
  const workflowSection = workflow === "superpowers" ? `
...
function setupProject({ projectRoot, workflow, architectureSkill, compoundRoot, instructions, force }) {
  ...
  outputs.push(installProjectHarnessScript(projectRoot));
...
function uninstallProject({ projectRoot, deleteSessions }) {
  ...
  for (const file of [
    ...
    path.join(projectRoot, ".harness", "scripts", "harness_session.py"),
  ]) {
    if (exists(file)) {
      fs.unlinkSync(file);
  ...
  const scriptsDir = path.join(projectRoot, ".harness", "scripts");
  if (exists(scriptsDir) && fs.readdirSync(scriptsDir).length === 0) {
    fs.rmdirSync(scriptsDir);
    outputs.push(`removed ${scriptsDir}`);
  }
  if (deleteSessions) {
    const sessions = path.join(projectRoot, DEFAULT_SESSION_ROOT);
    if (exists(sessions)) {
...
function commandDoctor() {
  const rows = [
    ...
    ["compound skill", exists(path.join(ROOT, "skills", "compound-engineering-capture", "SKILL.md")) ? "present" : "missing"],
